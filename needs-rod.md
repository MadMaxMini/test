...
| Date | Item | Context |
|------|------|---------|
| 2026-05-19 | **Sorting Hat cutover: Chase credit routing** | Telegram DM sent (msg 306). Question: do credit cards (3160, 3202) currently flow to sheets automatically, or are those done manually? Determines whether `chase_credit` PDFs route to mortgage folder (option 1) or quarantine (option 2). Decision needed before Session 2 cutover. |
| 2026-05-19 | **Sorting Hat cutover: alert verbosity (team)** | Discussion doc at `dakota-software/docs/sheets/sorting-hat-alert-discussion.md` sent to Dakota group via iMessage. 4 options A-D. Waiting on Doc, Devon, Sharon replies. Rod has final call. Affects whether per-file alerts fire on unknowns or batch into daily digest. |
